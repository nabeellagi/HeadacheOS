...
export default function MusicPlayer() {
  ...
  const handleSpeedSpin = () => {
    const newRate =
      SPEED_OPTIONS[Math.floor(Math.random() * SPEED_OPTIONS.length)];
    ...
    if (rateRef.current) {
      gsap.fromTo(
        ...
        },
        {
          ...
          onComplete: () =>
            gsap.to(rateRef.current, { scale: 1, duration: 0.2 }),
        }
      );
    }
  };
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full min-h-screen bg-base-200 p-4 animate-fadeIn font-['Roboto_Slab']">
      {isLoading && (
        <div className="absolute top-0 left-0 right-0 bottom-0 flex items-center justify-center bg-black bg-opacity-60 text-white text-xl z-50">
          Loading...
        </div>
      )}
      <div className="backdrop-blur-md bg-base-300/80 p-6 rounded-xl shadow-2xl w-full max-w-lg border border-base-content">
        ...
        <div className="text-center mb-4">
          ...
          <p className="text-md text-gray-500">
            by {songList[currentIndex].creator}
          </p>
        </div>
        ...
        <div className="flex flex-col gap-4">
          <div className="bg-base-100/90 p-4 rounded-md border border-base-content shadow-inner">
            <p className="font-semibold mb-2 flex items-center gap-2">
              <Icon icon="ic:round-volume-up" className="text-xl" /> Volume
              Slots (Total: {totalVolume}%)
            </p>
            <div className="grid grid-cols-5 gap-2">
              {volumeSlots.map((val, i) => (
                <div
                  ...
                  data-tip={locked[i] ? "Locked" : "Unlocked"}
                >
                  <button
                    ...
                    className={`btn btn-sm flex flex-col items-center justify-center transition-all text-lg ${
                      locked[i] ? "btn-warning" : "btn-outline"
                    }`}
                  >
                    <span>{val}</span>
                  </button>
          ...
          <div className="flex justify-center space-x-4 mt-6">
            {[0, 1, 2].map((i) => (
              <div
                key={i}
                className="w-12 h-12 overflow-hidden border border-base-content bg-base-100 rounded shadow-lg"
              >
                <div
                  ref={(el) => (spinRef.current[i] = el)}
                  className="transition-transform"
                >
                  {[...Array(songList.length)].map((_, j) => (
                    <div
                      key={j}
                      className="text-center text-xl h-10 leading-10 font-bold"
                    >
                      {j}
                    </div>
                  ))}
          ...
          <div className="mt-4 text-center">
            ...
            <div className="flex justify-center gap-2 items-center">
              <button
                ...
                onClick={handleSpeedSpin}
              >
                <Icon icon="mdi:fast-forward-outline" className="mr-1" /> Spin
                Speed
              </button>
              <span ref={rateRef} className="text-lg font-bold">
                {playbackRate}x
              </span>
            </div>
          </div>
          ...
          <canvas
            ...
            width={600}
            height={150}
            className="mt-6 border border-base-content rounded w-full"
          ></canvas>
        </div>
      </div>
    </div>
